...
async def test_create_and_get_patrol(db):
    ...
    assert fetched.program_name == patrol.program_name
...
async def test_list_patrols(db):
    store = PatrolsStore(db)
    patrol1 = Patrol(id=str(uuid6.uuid7()), program_name="Patrol 1")
    patrol2 = Patrol(id=str(uuid6.uuid7()), program_name="Patrol 2")
    ...
    names = [p.program_name for p in patrols]
...
async def test_update_patrol(db):
    store = PatrolsStore(db)
    patrol = Patrol(id=str(uuid6.uuid7()), program_name="Original Name")
    await store.create(patrol)
    
    patrol.program_name = "Updated Name"
    patrol.members = [PatrolMember(id=str(uuid6.uuid7()), name="Charlie")]
    await store.update(patrol)
    
    fetched = await store.get(patrol.id)
    assert fetched.program_name == "Updated Name"
...
async def test_delete_patrol(db):
    store = PatrolsStore(db)
    patrol = Patrol(id=str(uuid6.uuid7()), program_name="To Be Deleted")
    await store.create(patrol)
